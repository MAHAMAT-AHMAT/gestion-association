...
export default function TransactionList() {
  ...
  // Filter transactions by donateurNom, reference, or modePaiement
  const filteredTransactions = transactions.filter((t) =>
    search
      ? (t.don &&
          "donateurNom" in t.don &&
          t.don.donateurNom?.toLowerCase().startsWith(search.toLowerCase())) ||
        (t.reference &&
          t.reference.toLowerCase().startsWith(search.toLowerCase())) ||
        (t.modePaiement &&
          t.modePaiement.toLowerCase().startsWith(search.toLowerCase()))
      : true
  );
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredTransactions.map((t) => (
                <tr key={t.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 border border-gray-200 text-sm">
                    {t.annee && "libelle" in t.annee ? t.annee.libelle : "-"}
                  </td>
                  <td className="px-6 py-4 border border-gray-200 text-sm">
                    {t.datePaiement}
                  </td>
