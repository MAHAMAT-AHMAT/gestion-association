...
export default function DonateurList() {
  ...
  // Filter donateurs by first letter or substring (nom, prenom, email, pays)
  const filteredDonateurs = donateurs.filter((d) =>
    search
      ? d.nom.toLowerCase().startsWith(search.toLowerCase()) ||
        d.prenom.toLowerCase().startsWith(search.toLowerCase()) ||
        d.email.toLowerCase().startsWith(search.toLowerCase()) ||
        d.pays.toLowerCase().startsWith(search.toLowerCase())
      : true
  );
              ...
              {filteredDonateurs.map((d) => (
                <tr key={d.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 truncate border border-gray-200">
                    {d.typeDonateur === "MORAL"
                      ? "Moral"
                      : d.typeDonateur === "FINANCIER"
                      ? "Financier"
                      : "Les deux"}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 truncate border border-gray-200">
